...
library(tidyverse)
library(microeco)
otu <- read.delim("~/Desktop/饥饿小龙虾文章/最新抽平数据/lefse数据/convsrefed.txt",header = TRUE,row.names = 1)
tax <- read.delim("~/Desktop/饥饿小龙虾文章/最新抽平数据/lefse数据/taxon.txt",header = TRUE,row.names = 1)
sampledata<-read.delim("~/Desktop/饥饿小龙虾文章/最新抽平数据/lefse数据/sampleconvsrefed.txt",header = TRUE,row.names = 1)
...
t1$plot_lefse_cladogram(use_taxa_num = 100, 
                        ...
                        node_size_offset = 1,
                        annotation_shape = 22,
                        annotation_shape_size = 5,color = c("#333399","#FF0000")
